...
//    //时间是unix的绝对时间戳(自1970-01-01以来的秒数) 或以秒为单位的时间戳
...
//
//        if (highestBid != 0) {
//            //返还出价时, 简单的直接调用highestBidder.send(highestBid)函数是有安全风险的,因为它有可能执行一个
//            //非信任合约. 更为安全的做法是让接收方自己提取金钱
...
//        if (amount > 0) {
//            //这里很重要, 首先要设置零值. 因为,作为接收调用的一部分, 接收者可以在send返回之前, 重新调用函数
...
//        //对于可与其他合约交互的函数(意味着它会调用其他函数或者发送以太币)
//        //一个好的指导方针是将其结构分为三个阶段
//        // 1.检查条件
//        // 2.执行动作(可能会改变条件)
//        // 3.与其他合约交互
